...
export const Edit = () => {
      ...
      async function fetchData() {
        try {
          const activoVulnerabilidadResponse = await axios.get(
            'http://localhost:8800/api/amenazasvulnerabilidades/total'
          );
  ...
  const SeleccionarActivo = ({ activo }) => {
      ...
      async function fetchData() {
        try {
          const activosResponse = await axios.get(
            "http://localhost:8800/api/activos"
          );
          setActivos(
          ...
          const vulnerabilidadesResponse = await axios.get(
            "http://localhost:8800/api/vulnerabilidades"
          );
          setVulnerabilidades(
          ...
          const amenazasResponse = await axios.get(
            "http://localhost:8800/api/amenazas"
          );
          setAmenazas(
          ...
          const amenazasVulnerabilidadesResponse = await axios.get("http://localhost:8800/api/amenazasvulnerabilidades");
    ...
    const obtenerFechaVulnerabilidad = async (vulnerabilidadCodigo) => {
      try {
        const fechaResponse = await axios.get(
          `http://localhost:8800/api/amenazasvulnerabilidades/fecha/${vulnerabilidadCodigo}`
        );
    ...
    const sacarIddeActivo = async (activo) => {
      try {
        const activoResponse = await axios.get(`http://localhost:8800/api/activos/nombre/${activo}`);
    ...
    const handleGuardarClick = async () => {
      try {
        ...
        window.location.href = "http://localhost:3000/gestionar/editar-vulnerabilidades";
      } catch (error) {
    ...
    const saveDataToApi = async (activoId) => {
      try {
        ...
        for (const data of dataToSend) {
          ...
          await axios.post("http://localhost:8800/api/vulnerabilidadActivo", data);
    ...
    const saveData2 = async (activoId) => {
      try {
        ...
        for (const data of dataToSend) {
          console.log("Enviando datos:", data);
          await axios.post("http://localhost:8800/api/valorimpacto", data);
          console.log("Datos enviados con éxito.");
        }
