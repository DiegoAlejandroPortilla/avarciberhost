...
const TablaGA = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:8800/api/activos');
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:8800/api/tiposactivos');
        // Assuming the response data is an array, you can set it to the 'datos' state
  ...
  const handleGuardar = async () => {
    if (modificarIndex !== -1) {
      ...
      await axios.put(`http://localhost:8800/api/activos/${modificarIndex}`, formulario);
      // Now update the 'datos' state with the updated data
      const response = await axios.get('http://localhost:8800/api/activos');
      setDatos(response.data);
    } else {
      await axios.post('http://localhost:8800/api/activos', formulario);
  ...
  const handleEliminar = async (index) => {
    ...
    await axios.delete(`http://localhost:8800/api/activos/${codigoToDelete}`);
