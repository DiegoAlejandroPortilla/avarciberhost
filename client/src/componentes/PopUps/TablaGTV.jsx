...
const TablaTA = () => {
  ...
  const handleGuardar = async () => {
    if (modificarIndex !== -1) {
      ...
      await axios.put(`http://localhost:8800/api/tiposvulnerabilidad/${modificarIndex}`, formulario);
      // Now update the 'datos' state with the updated data
      const response = await axios.get('http://localhost:8800/api/tiposvulnerabilidad');
      setDatos(response.data);
    } else {
      // Agregar un nuevo dato
      await axios.post('http://localhost:8800/api/tiposvulnerabilidad', formulario);
  ...
  const handleEliminar = async (index) => {
    ...
    await axios.delete(`http://localhost:8800/api/tiposvulnerabilidad/${codigoToDelete}`);
