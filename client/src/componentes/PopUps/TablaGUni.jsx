...
const TablaUnidades = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:8800/api/unidades');
  ...
  const handleGuardar = async () => {
    if (modificarIndex !== -1) {
        ...
        await axios.put(`http://localhost:8800/api/unidades/${modificarIndex}`, formulario);
        // Now update the 'datos' state with the updated data
        const response = await axios.get('http://localhost:8800/api/unidades');
        setDatos(response.data);
      } else {
        // Agregar un nuevo dato
        await axios.post('http://localhost:8800/api/unidades', formulario);
  ...
  const handleEliminar = async (index) => {
    ...
    await axios.delete(`http://localhost:8800/api/unidades/${codigoToDelete}`);
