...
export const Edit = () => {
  ...
  const SeleccionarActivo = ({ activo }) => {
    ...
    useEffect(() => {
      async function fetchData() {
        try {
          ...
          const amenazasVulnerabilidadesResponse = await axios.get("https://savarciberespe-production.up.railway.app/api/amenazasvulnerabilidades");
    ...
    const sacarIddeActivo = async (activo) => {
      try {
        const activoResponse = await axios.get(`https://savarciberespe-production.up.railway.app/api/activos/nombre/${activo}`);
