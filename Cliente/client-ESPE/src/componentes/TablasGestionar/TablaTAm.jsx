...
const TablaTAm = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get('https://savarciberespe-production.up.railway.app/api/tipoamenaza');
  ...
  const handleGuardar = async () => {
    if (modificarIndex !== -1) {
      ...
      await axios.put(`https://savarciberespe-production.up.railway.app/api/tipoamenaza/${modificarIndex}`, formulario);
      ...
      try {
        ...
        await axios.post('https://savarciberespe-production.up.railway.app/api/tipoamenaza', formulario);
  ...
  const handleModificar = (index) => {
    setModificarIndex(datos[index].TIA_CODIGO);
    setFormulario({
      Descripcion: datos[index].TIA_DESCRIPCION,
    });
    setModalOpen(true);
  };
  ...
  const handleEliminar = async (index) => {
    ...
    await axios.delete(`https://savarciberespe-production.up.railway.app/api/tipoamenaza/${codigoToDelete}`);
          ...
          <Table sx={{ width: '97%', margin: '20px 10px 10px 10px', borderCollapse: 'collapse', border: '1px solid #ddd' }}>
            <TableBody>
              ...
              {datos.map((dato, index) => (
                <TableRow key={index}>
                  <TableCell colSpan={2} align="center">{dato.TIA_CODIGO}</TableCell>
                  <TableCell colSpan={2} align="center" >{dato.TIA_DESCRIPCION}</TableCell>
