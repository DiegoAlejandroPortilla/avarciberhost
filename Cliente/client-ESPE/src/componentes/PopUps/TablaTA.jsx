...
const TablaTA = () => {
  ...
  const handleGuardar = async () => {
    if (modificarIndex !== -1) {
      ...
      await axios.put(`https://savarciberespe-production.up.railway.app/api/tiposactivos/${modificarIndex}`, formulario);
      // Now update the 'datos' state with the updated data
      const response = await axios.get('https://savarciberespe-production.up.railway.app/api/tiposactivos');
      setDatos(response.data);
    } else {
      // Agregar un nuevo dato
      await axios.post('https://savarciberespe-production.up.railway.app/api/tiposactivos', formulario);
  ...
  const handleEliminar = async (index) => {
    ...
    await axios.delete(`https://savarciberespe-production.up.railway.app/api/tiposactivos/${codigoToDelete}`);
