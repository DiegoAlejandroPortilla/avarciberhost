...
export const Edit = () => {
      ...
      async function fetchData() {
        try {
          const activoVulnerabilidadResponse = await axios.get(
            'https://savarciberpetro-production.up.railway.app/api/amenazasvulnerabilidades/total'
          );
          ...
          const mappedData = activoVulnerabilidadResponse.data.map((item) => {
            ...
            return {
              ...
              promedio: parseFloat(item.PromedioGrupo).toFixed(3),
  ...
  const SeleccionarActivo = ({ activo }) => {
      ...
      async function fetchData() {
        try {
          const activosResponse = await axios.get(
            "https://savarciberpetro-production.up.railway.app/api/activos"
          );
          setActivos(
          ...
          const vulnerabilidadesResponse = await axios.get(
            "https://savarciberpetro-production.up.railway.app/api/vulnerabilidades"
          );
          setVulnerabilidades(
          ...
          const amenazasResponse = await axios.get(
            "https://savarciberpetro-production.up.railway.app/api/amenazas"
          );
          setAmenazas(
          ...
          const amenazasVulnerabilidadesResponse = await axios.get("https://savarciberpetro-production.up.railway.app/api/amenazasvulnerabilidades");
    ...
    const obtenerFechaVulnerabilidad = async (vulnerabilidadCodigo) => {
      try {
        const fechaResponse = await axios.get(
          `https://savarciberpetro-production.up.railway.app/api/amenazasvulnerabilidades/fecha/${vulnerabilidadCodigo}`
        );
    ...
    const sacarIddeActivo = async (activo) => {
      try {
        const activoResponse = await axios.get(`https://savarciberpetro-production.up.railway.app/api/activos/nombre/${activo}`);
        ...
        if (activoResponse.data.hasOwnProperty('ACT_CODIGO')) {
          const actCodigo = activoResponse.data.ACT_CODIGO;
    ...
    const handleGuardarClick = async () => {
      try {
        ...
        window.location.href = "https://avarciber-e4eb5.web.app/gestionar/editar-vulnerabilidades";
      } catch (error) {
    ...
    const saveDataToApi = async (activoId) => {
      try {
        ...
        for (const data of dataToSend) {
          ...
          await axios.post("https://savarciberpetro-production.up.railway.app/api/vulnerabilidadActivo", data);
    ...
    const saveData2 = async (activoId) => {
      try {
        ...
        for (const data of dataToSend) {
          console.log("Enviando datos:", data);
          await axios.post("https://savarciberpetro-production.up.railway.app/api/valorimpacto", data);
          console.log("Datos enviados con éxito.");
        }
