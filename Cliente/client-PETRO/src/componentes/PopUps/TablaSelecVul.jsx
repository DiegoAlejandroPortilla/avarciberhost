...
const TablaGA = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get('https://savarciberpetro-production.up.railway.app/api/vulnerabilidades');
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get('https://savarciberpetro-production.up.railway.app/api/tiposvulnerabilidad');
        // Assuming the response data is an array, you can set it to the 'datos' state
  ...
  const handleGuardar = async () => {
    if (modificarIndex !== -1) {
      ...
      await axios.put(`https://savarciberpetro-production.up.railway.app/api/vulnerabilidades/${modificarIndex}`, {
        ...formulario,
        Tipo: formulario.TipoId, // Use TipoId for the Tipo value
      });
      // Now update the 'datos' state with the updated data
      const response = await axios.get('https://savarciberpetro-production.up.railway.apps/api/vulnerabilidades');
      setDatos(response.data);
    } else {
      await axios.post('https://savarciberpetro-production.up.railway.app/api/vulnerabilidades',{
        ...formulario,
        Tipo: formulario.TipoId, // Use TipoId for the Tipo value
      });
  ...
  const handleModificar = (index) => {
    const formattedFechaInicio = dayjs(datos[index].VUL_FECHA_INICIO).format('YYYY-MM-DD');
    
    const selectedTipo = tipos.find((tipo) => tipo.TVU_DESCRIPCION === datos[index].TVU_CODIGO);
    
    setModificarIndex(datos[index].VUL_CODIGO);
    setFormulario({
      TipoId: selectedTipo ? selectedTipo.TVU_CODIGO : '',
      Tipo: selectedTipo ? selectedTipo.TVU_DESCRIPCION : '',
  ...
  const handleEliminar = async (index) => {
    const codigoToDelete = datos[index].VUL_CODIGO;

    // Make the DELETE request to delete the data
    await axios.delete(`https://savarciberpetro-production.up.railway.app/api/vulnerabilidades/${codigoToDelete}`);
            ...
            <TableBody>
              ...
              {datos.map((dato, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell colSpan={2} align="center">
                    {dato.VUL_FECHA_INICIO.split('T')[0]}
                  </TableCell>
                  <TableCell colSpan={2} align="center">
                  ...
                  <TableCell colSpan={2} align="center">
                    {dato.VUL_FECHA_INACTIVIDAD?dato.VUL_FECHA_INACTIVIDAD.split('T')[0]:dato.VUL_FECHA_INACTIVIDAD}
                  </TableCell>
                  <TableCell colSpan={2} align="center">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
        ...
        >
          ...
          <Autocomplete
            ...
            options={tipos.map((tipo) => tipo.TVU_DESCRIPCION)} 
            sx={{ width: 300 }}
  onChange={(event, newValue) => {
    const selectedTipo = tipos.find((tipo) => tipo.TVU_DESCRIPCION === newValue);
    setFormulario({
      ...formulario,
      TipoId: selectedTipo ? selectedTipo.TVU_CODIGO : '',
