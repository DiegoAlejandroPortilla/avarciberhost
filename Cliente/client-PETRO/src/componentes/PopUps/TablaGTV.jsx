...
const TablaTA = () => {
  ...
  const handleGuardar = async () => {
    if (modificarIndex !== -1) {
      ...
      await axios.put(`https://savarciberpetro-production.up.railway.app/api/tiposvulnerabilidad/${modificarIndex}`, formulario);
      // Now update the 'datos' state with the updated data
      const response = await axios.get('https://savarciberpetro-production.up.railway.app/api/tiposvulnerabilidad');
      setDatos(response.data);
    } else {
      // Agregar un nuevo dato
      await axios.post('https://savarciberpetro-production.up.railway.app/api/tiposvulnerabilidad', formulario);
  ...
  const handleEliminar = async (index) => {
    const codigoToDelete = datos[index].TVU_CODIGO;

    // Make the DELETE request to delete the data
    await axios.delete(`https://savarciberpetro-production.up.railway.app/api/tiposvulnerabilidad/${codigoToDelete}`);
